...
class ProdukLayananController extends Controller
{
    public function head_pl()
    {
        ...
        $pl = ProdukLayanan::where('user_id', '3')->where('is_active', 'Active')->first();
        $pls = DetailLayanan::where('user_id', '3')->where('is_active', 'Active')->orderBy('created_at', 'DESC')->paginate(6);
        $dpls = DetailLayanan::where('user_id', '3')->where('is_active', 'Active')->get();
    ...
    public function detail($id)
    {
        ...
        $pl = ProdukLayanan::where('user_id', '3')->where('is_active', 'Active')->first();
        $dtllayanan = DetailLayanan::where('user_id', '3')->where('is_active', 'Active')->where('id', $id)->first();
